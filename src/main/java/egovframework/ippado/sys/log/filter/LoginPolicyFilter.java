package egovframework.ippado.sys.log.filter;
...
public class LoginPolicyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (id == null) {
			((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/sys/user/loginUsr.do");
		}
		...
		try {
			...
			((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/sys/user/loginUsr.do?login_error=1");
		}
