...
function fncSelectAuthorList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sys/author/AuthorList.do'/>";
    document.listForm.submit();
}

function fncSelectAuthor(author) {
    document.listForm.authorCode.value = author;
    document.listForm.action = "<c:url value='/sys/author/Author.do'/>";
    document.listForm.submit();     
}

function fncAddAuthorInsert() {
    location.replace("<c:url value='/sys/author/AuthorInsertView.do'/>"); 
}
...
function fncAuthorDeleteList() {
    ...
    if(fncManageChecked()) {    
        if(confirm("삭제하시겠습니까?")) {
            document.listForm.action = "<c:url value='/sys/author/AuthorListDelete.do'/>";
            document.listForm.submit();
        } 
    }
}
...
function fncAddAuthorView() {
    document.listForm.action = "<c:url value='/sys/author/AuthorUpdate.do'/>";
    document.listForm.submit();     
}
    ...
    <div id="container">
        ...
        <!-- //좌측메뉴 끝 -->
            ...
            <div id="content">
                ...
                <!-- 검색 필드 박스 시작 -->
                <form:form id="listForm" name="listForm" action="<c:url value='/sys/author/AuthorList.do'/>" method="post">
                <div id="search_field">
                ...
                <!-- //페이지 네비게이션 끝 -->  
				    <div align="right">
				        <input type="text" name="message" value="<c:out value='${message}'/>" style="width:100%;border:0;margin-left:20px;font-size:12px"  readonly="readonly" title="메시지"/>
				    </div>
				</c:if>
