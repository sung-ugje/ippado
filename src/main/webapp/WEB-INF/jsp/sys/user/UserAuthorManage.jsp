<%--
  Class Name : UserAuthorManage.jsp
  Description : UserAuthorManage List 화면
...
function fncSelectAuthorGroupList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sys/user/UserAuthorList.do'/>";
...
function fncAddAuthorGroupInsert() {
    ...
    if(confirm("등록하시겠습니까?")) {
        document.listForm.action = "<c:url value='/sys/user/UserAuthorInsert.do'/>";
        document.listForm.submit();
    }
}
...
function fncAuthorGroupDeleteList() {
    ...
    if(confirm("삭제하시겠습니까?")) {
        document.listForm.action = "<c:url value='/sys/user/UserAuthorDelete.do'/>";
        document.listForm.submit(); 
    }
}
...
<div id="wrap">
    ...
    <div id="container">
        ...
        <!-- //좌측메뉴 끝 -->
            <!-- 현재위치 네비게이션 시작 -->
            <div id="content">
            <form:form id="listForm" name="listForm" action="<c:url value='/sys/user/UserAuthorList.do'/>" method="post">
                <div id="cur_loc">
                                ...
                                <ul id="search_first_ul">
                                    <li>
    					                ...
    					                <select id="searchCondition" name="searchCondition" onchange="onSearchCondition()" title="조회조건">
    					                    <option value="1" <c:if test="${userAuthorVO.searchCondition == '1'}">selected</c:if> >사용자 ID</option>
    					                    <option value="2" <c:if test="${userAuthorVO.searchCondition == '2'}">selected</c:if> >사용자 명</option>
    					                    <option value="3" <c:if test="${userAuthorVO.searchCondition == '3'}">selected</c:if> >그룹</option>
    					                </select>
                                    </li>
                                    <li>
        								<input name="searchKeyword" type="text" value="<c:out value='${userAuthorVO.searchKeyword}'/>" size="25" title="검색" onkeypress="press();"/>
                ...
                <div class="default_tablestyle">
                    ...
                    <!-- loop 시작 -->                                
                      <tr>
                        <td nowrap="nowrap"><input type="checkbox" name="delYn" class="check2" title="선택"><input type="hidden" name="checkId" value="<c:out value="${userAuthor.uniqId}"/>"/></td>
                        <td nowrap="nowrap"><c:out value="${userAuthor.userId}"/></td>
                        <td nowrap="nowrap"><c:out value="${userAuthor.userNm}"/></td>
                        <td nowrap="nowrap"><c:out value="${userAuthor.mberTyNm}"/><input type="hidden" name="mberTyCode" value="${userAuthor.mberTyCode}"/></td>
                        <td nowrap="nowrap">
                            <select name="authorManageCombo" title="권한">
					            <c:forEach var="authorManage" items="${authorManageList}" varStatus="status">
					                <option value="<c:out value="${authorManage.authorCode}"/>" <c:if test="${authorManage.authorCode == userAuthor.authorCode}">selected</c:if> ><c:out value="${authorManage.authorNm}"/></option>
					            </c:forEach>
					        </select>
                        </td>
                        <td nowrap="nowrap"><c:out value="${userAuthor.regYn}"/><input type="hidden" name="regYn" value="<c:out value="${userAuthor.regYn}"/>"></td>
                      </tr>
                     </c:forEach>
                     <c:if test="${empty userAuthorList}">
                        <tr>
                ...
                <!-- 페이지 네비게이션 시작 -->
                <c:if test="${!empty userAuthorVO.pageIndex }">
                    <div id="paging_div">
